...
create_departments_table = """
    create table if not exists departments (
        ...
        foreign key (f_name) references faculties (f_name) on delete cascade on update cascade
    );
"""
...
#set on delete cascade on update cascade and on update cascade to synchronize data
create_classes_table = """
    create table if not exists classes (
        ...
        foreign key (s_ID) references subjects (s_ID) on delete cascade on update cascade,
        foreign key (l_ID) references lecturers (l_ID) on delete cascade on update cascade
    );
"""
...
create_books_table = """
    create table if not exists books (
        ...
        status char(40),
        lib_ID char(40),
        foreign key (lib_ID) references libraries (lib_ID) on delete cascade on update cascade
    );
"""
...
create_enroll_in_table = """
    create table if not exists enroll_in (
        ...
        foreign key (c_ID, semester) references classes (c_ID, semester) on delete cascade on update cascade
...
create_participate_in_table = """
    create table if not exists participate_in (
        ...
        stu_ID integer not null,
        bonus_score integer,
        foreign key (pro_ID) references projects (pro_ID) on delete cascade on update cascade
        foreign key (stu_ID) references students (stu_ID) on delete cascade on update cascade
    );
"""
